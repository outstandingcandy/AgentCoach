"""Faithful port of upstream `mguti97/PnLCalib` `utils/utils_calib.py`.

Differences from upstream — additions only, no logic edits:

1. ``FramebyFrameCalib.__init__`` accepts ``pitch_dims`` (dict of
   SoccerPitch field overrides). The literal pitch tables at the top of
   upstream's file (105×68 FIFA) are replaced by parametric tables built
   via ``build_keypoint_table``. Defaults are FIFA-spec, so FIFA-spec
   videos behave identically to upstream.
2. ``get_keypoints_subsets`` and ``lines_consensus``/``line_optimizer``
   read pitch tables from ``self`` instead of module globals.
3. NaN coords (penalty arc not reaching pa front line on small kids
   pitches) are skipped instead of being written to the subset dicts.
4. ``change_plane_coords`` uses ``self.pitch_length / self.pitch_width``
   instead of the upstream ``105 / 68`` defaults.

All keypoint IDs in dictionaries (``kp_dict``, ``lines_dict``) are
**1-indexed**, matching upstream. Use ``id_mapping.PnLCalibIdMap`` at
boundaries to convert from project 0-indexed names.
"""
...
def rotation_matrix_to_pan_tilt_roll(rotation):
    ...
    Two solutions exist; pick the one with smallest |roll|.
    """
...
class FramebyFrameCalib:
    def __init__(self, iwidth=960, iheight=540, denormalize=False, pitch_dims=None):
        ...
        self.alpha = 0.7  # weight for line_optimizer

        # Parametric pitch tables. The crossbar z = -goal_height is baked
        # into ``_kp_z_for_id`` (only the 4 NON_GROUND IDs use it).
        table = build_keypoint_table(pitch_dims)
        self._kp_table: list[tuple[float, float]] = table["keypoint_world_coords_2D"]
        self._kp_aux_table: list[tuple[float, float]] = table["keypoint_aux_world_coords_2D"]
        self._line_table: list = table["line_world_coords_3D"]
        self._non_ground_ids: set[int] = table["non_ground_ids"]
        self.pitch_length: float = table["pitch_length"]
        self.pitch_width: float = table["pitch_width"]
        self.goal_height: float = table["goal_height"]

    # ------------------------------------------------------------------
    # ID -> world lookup helpers (replace upstream module-level lists).
    # ------------------------------------------------------------------
    def _kp_world_xy(self, kp_id: int) -> tuple[float, float]:
        """Look up the (x, y) world coord for a 1-indexed upstream kp id."""
        if kp_id <= 57:
            return self._kp_table[kp_id - 1]
        return self._kp_aux_table[kp_id - 1 - 57]

    def _kp_z_for_id(self, kp_id: int) -> float:
        return -self.goal_height if kp_id in self._non_ground_ids else 0.0
    ...
    def get_keypoints_subsets(self):
        ...
        non_ground = self._non_ground_ids
        max_aux = 57 + len(self._kp_aux_table)

        for kp in list(self.keypoints_dict.keys()):
            # Drop ids the table can't resolve (would IndexError below).
            if kp < 1 or kp > max_aux:
                continue

            wp = self._kp_world_xy(kp)
            if not _is_finite_xy(wp):
                # NaN aux on small pitches where penalty arc doesn't reach
                # pa front line; upstream silently relied on FIFA dims so
                # never hit this — drop instead of poisoning the subsets.
                continue

            zw = -self.goal_height if kp in non_ground else 0.0
            entry = {
                'xi': self.keypoints_dict[kp]['x'],
                'yi': self.keypoints_dict[kp]['y'],
                'xw': wp[0], 'yw': wp[1], 'zw': zw,
            }
            full[kp] = entry
            if kp <= 30:
                main[kp] = dict(entry)
            if kp not in non_ground:
                ground_plane[kp] = dict(entry)
    ...
    def change_plane_coords(self):
        """Reverse upstream's ``z -> -x`` reparam after Zhang calibration.

        Uses ``self.pitch_length`` / ``self.pitch_width`` instead of the
        upstream FIFA literals.
        """
        R = np.array([[0, 0, -1], [0, 1, 0], [1, 0, 0]])
        self.rotation = self.rotation @ R
        if self.ord_pts[0] == 1:
            self.position = np.linalg.inv(R) @ self.position + np.array([-self.pitch_length / 2.0, 0, 0])
    ...
    def lines_consensus(self, threshold=100):
        ...
        if plane_normal is not None:
            for key, value in self.lines_dict.items():
                ...
                if key < 1 or key > len(self._line_table):
                    continue
                wp1, wp2 = self._line_table[key - 1]
                ...
                proj1 /= proj1[-1]
                proj2 /= proj2[-1]
                d1 = point_to_line_distance(proj1, proj2, np.array([x1, y1]))
                d2 = point_to_line_distance(proj1, proj2, np.array([x2, y2]))

                if d2 <= threshold and d1 <= threshold:
    ...
    def line_optimizer(self, vector, img_pts, obj_pts):
        ...
        err = []
        for _ in range(len(img_pts) + len(self.lines_dict_cons)):
    ...
    def get_cam_params(self, mode='full', use_ransac=0, refine=False, refine_w_lines=False):
        ...
        if refine:
            if not np.isnan(rep_err):
                ...
                rvecs, tvecs = cv2.solvePnPRefineLM(
                    obj_pts, img_pts, self.calibration, dist, rvec, tvec,
        ...
        if refine_w_lines:
            if not np.isnan(rep_err):
                ...
                res = least_squares(
                    self.line_optimizer, vector,
                    verbose=0, ftol=1e-4, x_scale="jac", method='trf',
    ...
    def estimate_calibration_matrix_from_plane_homography(self, homography):
        ...
        # principal point estimated by this method is noisy; keep at image center
    ...
    def get_homography_from_ground_plane(self, use_ransac=5., inverse=False, refine_lines=False):
        ...
        if self.position is not None and refine_lines:
            self.lines_consensus_ground()
            vector = H.flatten()[:-1]
            res = least_squares(
                self.line_optimizer_ground, vector,
                verbose=0, ftol=1e-4, x_scale="jac", method='lm',
                args=(img_pts, obj_pts),
            )
            vector_opt = res['x']
            if not any(np.isnan(vector_opt)):
            ...
            for use_ransac in [0, 5, 10, 15, 25, 50]:
                ...
                if ret:
                    final_results.append({
                        'mode': mode, 'use_ransac': use_ransac, 'rep_err': ret,
    ...
    def heuristic_voting_ground(self, refine_lines=False, th=5.):
        ...
        for use_ransac in [0, 5, 10, 15, 25, 50]:
            ...
            if H is not None:
                final_results.append({'use_ransac': use_ransac, 'rep_err': ret, 'homography': H})
