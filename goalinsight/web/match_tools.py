...
TOOL_SCHEMAS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "custom aggregations). Match data is pre-uploaded as JSON:\n"
            "\n"
            f"  {SANDBOX_DATA_DIR}/tracks.json\n"
            "    dict[frame_str -> list[track]]; each track has:\n"
            "      track_id (str, e.g. 'A-14'),\n"
            "      player_id (str, same as track_id once consolidated),\n"
            "      team ('team_A' | 'team_B' | 'referee'),\n"
            "      role ('player' | 'goalkeeper' | 'referee'),\n"
            "      jersey_number (int|null), jersey_confidence (float),\n"
            "      bbox [x1,y1,x2,y2] in pixels,\n"
            "      pitch_position [x_m, y_m] (meters; (0,0) at pitch centre,\n"
            "         x along length, y along width).\n"
            "    Use pitch_position for distance/speed; bbox is image-space.\n"
            "\n"
            f"  {SANDBOX_DATA_DIR}/ball_tracks.json\n"
            "    dict[frame_str -> ball]; ball has:\n"
            "      bbox, center [u,v] (pixels), pitch_position [x_m,y_m],\n"
            "      height (m), on_ground (bool), position_3d [x,y,z] (m),\n"
            "      predicted (bool — True when interpolated, not detected).\n"
            "\n"
            f"  {SANDBOX_DATA_DIR}/team_assignments.json\n"
            "    dict[player_id -> 'team_A'|'team_B'|'referee'].\n"
            "\n"
            f"  {SANDBOX_DATA_DIR}/events.json\n"
            "    list[event]; each event has:\n"
            "      event_id, type ('possession'|'pass'|'shot'|'goal'|"
            "'carry'|'tackle'|'interception'),\n"
            "      frame (int), match_time (seconds, float),\n"
            "      player_id, team_id, start_frame, end_frame,\n"
            "      start_position [x_m,y_m], end_position [x_m,y_m],\n"
            "      confidence (float), metadata (dict, type-specific).\n"
            "\n"
            "FPS and pitch dimensions are NOT in these files — read them "
            "from the schema brief in the system prompt (or compute "
            "frames-to-seconds via the FPS line there). Frames in tracks/"
            "ball_tracks may be sparse (process_fps < video fps); when "
            "integrating distance, treat consecutive sample gaps via "
            "frame indices, and clip implausible jumps (>5 m between "
            "samples) as ID-switch noise.\n"
            "\n"
