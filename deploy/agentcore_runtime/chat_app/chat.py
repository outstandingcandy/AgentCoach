...
SYSTEM_PROMPT = (
    ...
    "The tool result returns each artifact as "
    "`{url, mime_type, embed_as}` where `embed_as` is a "
    "ready-made markdown string like `![name](https://...)`. "
    "When you want to show the image in your answer, paste the "
    "`embed_as` string verbatim. Do NOT construct your own "
    "markdown image — the chat UI only renders the URLs the "
    "tool gives you. Use plots only when the user asks for a "
    "visualisation or distribution; for simple numbers use the "
    "dedicated stats tools.\n"
...
class ChatEngine:
    ...
    def stream(
        ...
        # Map of relative artifact paths (`output/foo.png`, `foo.png`) to
        # the presigned URL run_python returned for that artifact. Used
        # at end-of-stream to repair Claude's tendency to write the
        # sandbox-relative path even when the system prompt + tool
        # schema both demand the full URL.
        ...
        for round_idx in range(MAX_TOOL_ROUNDS):
                ...
                for b in ordered:
                    ...
                    if b.get("name") == "run_python":
                        for a in (result.get("artifacts") or []):
                            ...
                            artifact_urls[tail] = url
                            artifact_urls[f"output/{tail}"] = url
                            artifact_urls[f"./output/{tail}"] = url
                    tool_results.append({
                        "type": "tool_result",
            ...
            # Before yielding, repair any `output/foo.png` references
            # the model wrote despite our prompt: replace with the
            # presigned URL we returned for that artifact. This avoids
            # a broken inline image in the UI when Claude shortcuts.
            full_text = "".join(t for _, t in text_deltas)
            if artifact_urls:
                full_text = _rewrite_artifact_paths(full_text, artifact_urls)
            if full_text:
                yield full_text
            return
...
def _rewrite_artifact_paths(text: str, urls: dict[str, str]) -> str:
    """Replace ``![alt](output/foo.png)`` with the presigned URL we
    returned in the matching run_python tool_result. Looks up by the
    URL portion of the markdown image; if no match, leaves the link
    alone so legitimate non-image links pass through.
    """
    def sub(m: "re.Match[str]") -> str:
        alt, link = m.group(1), m.group(2)
        if link.startswith(("http://", "https://", "/")):
            return m.group(0)
        # Try exact, basename, and bare filename lookups.
        candidates = [link, link.lstrip("./"), link.rsplit("/", 1)[-1]]
        for k in candidates:
            if k in urls:
                logger.info(
                    "rewrote markdown image url: %r -> %s…", link, urls[k][:60],
                )
                return f"![{alt}]({urls[k]})"
        return m.group(0)
    return _MD_IMG_RE.sub(sub, text)
